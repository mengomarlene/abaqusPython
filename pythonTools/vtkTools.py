...
def displayUGridWithField(fileName,fieldName):
    # Shows vtu
    reader = vtk.vtkUnstructuredGridReader()
    reader.SetFileName(fileName)
    reader.Update()
    cellData  = reader.GetOutput().GetCellData()
    range = cellData.GetScalars().GetRange()
    lut = vtk.vtkLookupTable()
    lut.SetScaleToLinear()
    lut.SetRange(range)
    lut.SetHueRange(0.6667,0.0)#blue to red
    lut.Build()
...
def produceVTKFile(odbPath,vtkFile,fieldName,notWantedParts=[],notWantedEleSets=[],vizu=True):
    if 'VM' in fieldName:
        dataType = 'dataVMField'
    elif 'pressure' in fieldName:
        dataType = 'dataPField'
    else: raise Exception("undefined field name")

    longTxt = '''
print "reading abaqus odb"
from abaqusPythonTools.abaqusVTKTools import %s as dataFieldNamed
nodeCoord,eleConnection,cellType,cellField = dataFieldNamed('%s',%r,%r)
print "writing vtk file"
from abaqusPythonTools.abaqusVTKTools import writeVtk
writeVtk('%s',nodeCoord,eleConnection,cellType,cdata=cellField,cdataName='%s')
        '''%(dataType,odbPath,notWantedParts,notWantedEleSets,vtkFile,fieldName)
...
def produceVTKFileNoElSet(odbPath,vtkFile,fieldName,notWantedParts=[],notWantedEleSets=[],vizu=True):
    if 'VM' in fieldName:
        dataType = 'dataVMField'
        fieldName = 'Shear_Stress'
    elif 'pressure' in fieldName:
        dataType = 'dataPField'
    else: raise Exception("undefined field name")
    longTxt = '''
print "reading abaqus odb"
from abaqusPythonTools.abaqusVTKToolsNoElSet import %s as dataFieldNamed
nodeCoord,eleConnection,cellType,cellField = dataFieldNamed('%s',%r,%r)
print "writing vtk file"
from abaqusPythonTools.abaqusVTKToolsNoElSet import writeVtk
writeVtk('%s',nodeCoord,eleConnection,cellType,cdata=cellField,cdataName='%s')
        '''%(dataType,odbPath,notWantedParts,notWantedEleSets,vtkFile,fieldName)
...
def produceVTKFileMaterial(odbPath,vtkFile,notWantedParts=[],notWantedEleSets=[],vizu=True):

    longTxt = '''
print "reading abaqus odb"
from abaqusPythonTools.abaqusVTKTools import materialField
nodeCoord,eleConnection,cellType,cellField = materialField('%s',%r,%r)
print "writing vtk file"
from abaqusPythonTools.abaqusVTKTools import writeVtk
writeVtk('%s',nodeCoord,eleConnection,cellType,cdata=cellField,cdataName='Material_Stiffness_[GPa]')
        '''%(odbPath,notWantedParts,notWantedEleSets,vtkFile)
