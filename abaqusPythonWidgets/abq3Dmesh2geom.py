...
# adapted from R. Feldbacher - Graz University of Technology
...
eleEdges=ypart.elementEdges
l=0
e20=len(eleEdges)/20
# read nodal data from ypart (copied model)
for i,edge in enumerate(eleEdges):
    points=list()
    for point in edge.getNodes():
        points.append(point.coordinates)
    xpart.WireSpline(points=points, mergeWire=ON,meshable=ON, smoothClosedSpline=ON)
    #vizu stuff
    l=l+1
    if l > e20:
        l=0
        print i+1,' of ',len(eleEdges),' wires completed'
...
eleFaces=ypart.elementFaces
l=0
n=list()
e20=len(eleFaces)/20
for i,faces in enumerate(eleFaces):
    edl=list()
    for edge in faces.getElemEdges():
        en=edge.getNodes()
...
# generate faces - try different settings and order
    ...
    if l > e20:
        l=0
        print i+1,' of ',len(eleFaces),' faces completed'
...
if len(n) > 0:
    ...
    for sFace in skippedFaces:
        edl=list()
        for edge in eleFaces[sFace].getElemEdges():
            en=edge.getNodes()
            if len(en)==3:
                co2=en[1].coordinates
            else:    
...
if setsYN=='Y' or setsYN=='y':
    for setKey in wpart.allSets.keys():
        print 'Generating Set:',setKey
        elfailed=0
        nofailed=0
# read element sets and try to find cells
        esettoadd=[]
        for ele in wpart.allSets[setKey].elements:
            wconn=ele.connectivity
# calculate mean coordinates to find point inside element/cell
            from operator import add
            meancoords=[0]*len(wpart.nodes[wconn[0]].coordinates)
            for node in wconn:
                meancoords=map(add,meancoords,wpart.nodes[node].coordinates)/len(wconn)
...
# read nodes sets and try to find vertices
        nsettoadd=[]
        for node in wpart.allSets[setKey].nodes:
            ntoadd=zpart.vertices.findAt(((node.coordinates), ),printWarning=FALSE)
            if not ntoadd:
                nofailed+=1
            nsettoadd+=(ntoadd,)
        if nsettoadd or esettoadd:
            zpart.Set(vertices=nsettoadd,cells=esettoadd,name=setKey)
        if elfailed!=0:
            print elfailed,'Cell(s) could not be added to set',setKey
        if nofailed!=0:
            print nofailed,'Point(s) could not be added to set',setKey
######## surfaces
...
# find faces on elements
        for j,ele in enumerate (elems):
            nodeList=list()
            conlen=len(ele.connectivity)
            if sides[j]==BOTHSIDES or sides[j]==SIDE1 or sides[j]==SIDE2:
                nodeList=ele.connectivity
            else:
                elnocon=facecon[elnonum.index(conlen)][int(str(sides[j])[4])-1]
                for no in elnocon:
                    nodeList.append(ele.connectivity[no])
# calculate mean coordinates to find faces in solid part
            from operator import add
            meancoords=[0]*len(wpart.nodes[nodeList[0]].coordinates)
            for node in nodeList:
                meancoords=map(add,meancoords,wpart.nodes[node].coordinates)/len(nodeList)
