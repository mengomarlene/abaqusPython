...
def residuals(p, modelScript, expData, withBounds=False):
    ...
    #
    # interopolation of the fe or exp data to evaluate those on the same number of points (so that it is possible to take the difference)
    # --------------
    import numpy as np
    # nb of data to interpolate on
    nx = min(len(expData[0]),len(feData[0]))/2
    # Generate a regular grid, xi, to interpolate the data on
    xi = np.linspace(min(expData[0]), max(expData[0]), nx)
    intExp = interpolateResults(expData,xi)
    intFE = interpolateResults(feData,xi)
    diff = (intExp - intFE)
    lstSq = np.sum(diff ** 2)/nx
    global NFeval
    NFeval += 1
