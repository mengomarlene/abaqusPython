...
def getParameters(_p={}):
    ...
    param['holzapfelParameters'] = (.02, 0.001, 6.86, 219.94, 0.)#ovine data - c10,D made up...
...
def createAnalysis(param):
    ...
    if 'homogeneous' not in param['inpFile']:
        initInc = 1e-4
        maxInc = .1
        if 'Bonded' not in param['inpFile']: initInc = 1e-6
    ...
    for i,p in enumerate(param['parts']):
        ...
        if 'ENDPLATE' in mat:
            ...
            #print nbLamellae,len(param['fiberOrientation']),type(param['fiberOrientation'])
