...
Tested with scanIP5.1 and 6.0 and abaqus 6.12 and 6.13
...
VERSIONS:
VERSION 1.1 - MENMMEN: parametrised seed size
...
def shellTo2DGeo(myModel,seedSize=0.5):
    ...
    for myPart in myModel.parts.values():
        ...
        #1/ find zMin...
        zCoord = [node.coordinates[2] for node in myPart.nodes]
        nodeLabels = [node.label for node in myPart.nodes]
        minZ = min(zCoord)
        ...
        for sa,secAss in enumerate(myPart.sectionAssignments): del myPart.sectionAssignments[sa]
    ...
    for sName in myModel.sections.keys():del myModel.sections[sName]
    
    ## add new parts to assembly - only after the initial instance has been deleted has they are not of the same type
    for part in myModel.parts.values():createInstanceAndAddtoAssembly(part,myAssembly,independent=False)
#-----------------------------------------------------
