...
class ValueExtractor:
    ...
    Methods:
        ...
        setComponant(componentLabel) - the field componant, no default
        setInvariant(invariant) - the field invariant, default is MAGNITUDE
    ...
    def __init__(self,odb,setName):
        ...
        self.invariant = MAGNITUDE
        self.sysC = None
        self.stepName = None
    #-----------------------------------------------------
    def setField(self,fieldKey):
        self.fieldKey = fieldKey
    def setComponant(self,componentLabel):
    ...
    def getEvolution_Nodal(self):
        return self.__getEvolution(self)
    def getEvolution_ElementNodal(self):
        return self.__getEvolution(self,position='EL_N')
    def getEvolution_ElementIP(self):
        return self.__getEvolution(self,position='EL_IP')
    def getFinalValue_Nodal(self):
        return self.__getFinalValue(self)
    def getFinalValue_ElementNodal(self):
        return self.__getFinalValue(self,position='EL_N')
    def getFinalValue_ElementIP(self):
        return self.__getFinalValue(self,position='EL_IP')
    #-----------------------------------------------------
    #-----------------------------------------------------
    def __getEvolution(self,position=None):
        ...
        values = self.__getValues(self,frameNo=frames,position=position)
        value = list()
        for frame in range(len(frames)):
            value.append(list())
            for ptValue in values[frame]: value[frame].append(ptValue.data)
        return value
    def __getFinalValue(self,position=None):
        if self.stepName is None:self.stepName = self.odb.steps.keys()[-1]
        values = self.__getValues(self,position=position)
        value = list()
        for ptValue in values: value.append(ptValue.data)
        return value
    #-----------------------------------------------------
    def __getValues(self,frameNo=-1,position=None):
        try:
            values = list()
            for frameNb in range(len(frameNo)):
                val = self.__getValues(frameNb,position)
                values.append(val)
        except(TypeError):
