...
def appliedDisplExtractors(odbName):
    ...
    if run:
        ...
        extSet = 'EXTERNALANNULUS'
        if extSet not in myAssembly.nodeSets.keys():
            #try:
            mySurface = mainPart.surfaces[extSet]
            nodeList = list()
            ...
            extSet = myAssembly.NodeSetFromNodeLabels(name = extSet, nodeLabels = (('PART-1-1',myNodes),)) 
        else:extSet=myAssembly.nodeSets[extSet]
            #except:raise Exception('I run out of a try')

        cylSystem = getDatumOnSymPlane(myOdb)
